...
namespace ServiceLayer.CheckoutServices
{
    ...
    {
        private List<OrderItem> _lineItems;

      
        public Guid UserId { get; private set; }

      
        public ImmutableList<OrderItem> LineItems => _lineItems.ToImmutableList();
        ...
        public void AddLineItem(OrderItem newItem)
        {
            _lineItems.Add(newItem);
        }

        public void UpdateLineItem(int itemIndex, OrderItem replacement)
        {
        ...
        public string EncodeForCookie()
        {
            ...
            {
                sb.AppendFormat(",{0},{1}", lineItem.ProductId, lineItem.ProductCount);
            }
            return sb.ToString();
        }
        ...
        private void DecodeCookieString(string cookieContent)
        {
            ...
            {
                _lineItems.Add(new OrderItem
                {
                    ProductId = int.Parse(parts[i * 2 + 1]),
                    ProductCount = short.Parse(parts[i * 2 + 2])
                });
            }
        }
    }
